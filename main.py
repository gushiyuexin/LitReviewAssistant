...
v0.1.1 20240507
代码优化
1. 优化Json_preprocess，现在该部分可以直接运行，无需手动调整；
2. 将Json_Info_Read中的部分可修改参数外置到txt文件中，这样可以方便后续更改
3. 优化了标题、期刊的存储逻辑，现在csv格式文件的显示过程中，不会因为标题和期刊中存在逗号而导致分段异常

v0.1.2 20240613
代码优化
1. 考虑了wos中，如果摘要部分缺失，非论文（J)导致的相关问题；
...
v0.1.4 20241016
功能优化：
1. 加入论文期刊的影响因子，这样可以更加合理的评估论文的影响力；
2. 提供cnki的excel到处文献的提取功能

v0.1.5 20241024
功能优化：
针对当前LLM可能导致的json文件没法整体输出的情况，调整了提示词，并设计了一次输出数量的选项

v0.2.0 20241101
新增功能：
1. 现在可以初步实现csv文件的聚类分析了
2. 调整了titleList.txt的格式以及配套代码，现在可以实现一级分类词条的撰写

v0.3.0 20241102
新增UI功能 - 徐子人同学

v0.3.1 20241108
1. 将综述/研究性论文分开，分别提供不同的提示词模板；
2. 增加文献上限设定参数，这样方便对于部分文献进行分析；
3. 新增基于影响力的筛选条件，可以优选高影响力的文献进行分析；
